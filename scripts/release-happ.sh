...
#   1. `workdir/calcy.happ` from a local `npm run build:happ:release` — used when
...
if [ $# -ge 1 ]; then
  ...
  happ="$tmp/wa/calcy.happ"
else
  happ="workdir/calcy.happ"
  if [ ! -f "$happ" ]; then
...
notes="Frozen canonical calcy.happ reused by every webhapp release to keep all installs on the same network. DNA sha256: $EXPECTED_SHA. Do NOT rebuild."
...
echo "Done. release-webhapp.yaml will download calcy.happ from $HAPP_TAG."
