<script lang="ts">
  ...
  import { LocaleType, LogLevel, LocaleService, Univer, UniverInstanceType, type IDocumentData, type ICommand, type IAccessor, type JSONXActions, TextXActionType, TextX, JSONX, ICommandService, UserManagerService , Tools, IUniverInstanceService, MemoryCursor, type DocumentDataModel} from '@univerjs/core';
  ...
  const iCommandService = injector.get(ICommandService);
  ...
  function hackRefresh() {
    ...
    univerAPI.executeCommand(insertCommand.id, insertCommand.params, {"fromCollab": true})
    ...
    const deleteCommand = {
      ...
      "type": 2,
        "params": {
          ...
          "debounce": true,
          "trigger": "doc.command.delete-text"
      }
    }
    univerAPI.executeCommand(deleteCommand.id, deleteCommand.params, {"fromCollab": true})
  }

  const updateDocument = async () => {
    console.log("updating document", $synState.spreadsheet)
    console.log("univer object", currentUniverEditable.getSnapshot())
    currentUniverEditable.reset($synState.spreadsheet)
    // let x = currentUniverEditable.getSnapshot()
    hackRefresh()
    // return null
  }
  ...
  onMount(async () => {
    // iCommandService.registerCommand({"id": "refresh2",
    //     "type": 2,
    //     handler: (accessor: IAccessor) => {
    //       console.log('click button');
    //       return true;
    //     }});
    // iCommandService.registerCommand({"id": "refresh1",
    //     "type": 2,
    //     handler: (accessor: IAccessor) => {
    //       console.log('click button');
    //       return true;
    ...
    univerAPI.onBeforeCommandExecute((command, options) => {
        ...
        // else if (command.id == "doc.mutation.rich-text-editing" && (command.params?.actions && command.params?.actions[1]?.e[1]?.body.dataStream == "+") || command.params?.actions[1]?.e[0].t == "d") {
        //   return null
        // }
        else if (command.id.includes("mutation")) {
          console.log("command", command, "options", options)
          
          let actions = extractActionsFromCommands([command])

          let copyOfSnapshot = cloneDeep(currentUniverEditable.getSnapshot().body)

          let applied = TextX.apply(copyOfSnapshot
          , actions)

          console.log("applied", applied)
          console.log("synstate", $synState.spreadsheet)
